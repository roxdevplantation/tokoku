...
function _render() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <!-- Riwayat Omzet — hapus terpisah dari transaksi -->
    <div class="card fade-in" style="margin-bottom:14px">
      <div class="sec-header" style="margin-bottom:10px">
        <div class="sec-title">🧾 Riwayat Omzet</div>
        ${omzetLogs.length ? `
      ...
      <div style="font-size:11px;color:var(--muted2);margin-bottom:12px;
                  ...
                  border-left:3px solid var(--accent)">
        💡 Omzet tidak ikut terhapus saat transaksi dihapus
      </div>
      <div id="omzet-log-list">
        ${omzetLogHtml}
      </div>
...
function _getOmzetLogs() {
  const logs = window.__db?.omzet_log ?? [];
...
function _omzetLogItem(o) {
  const badge = o.metode === 'cash'
    ? `<span style="font-size:10px;font-weight:700;padding:2px 7px;
                    border-radius:20px;background:var(--green)22;
                    color:var(--green)">CASH</span>`
    : `<span style="font-size:10px;font-weight:700;padding:2px 7px;
                    border-radius:20px;background:var(--yellow)22;
                    color:var(--yellow)">KREDIT</span>`;
  return `
    <div id="omzet-item-${o.id}"
      style="display:flex;align-items:center;gap:10px;
             padding:10px 0;border-bottom:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
          <span style="font-size:13px;font-weight:700">${o.pelanggan}</span>
          ${badge}
        </div>
        <div style="font-size:11px;color:var(--muted);font-family:var(--mono)">
          ${fmtDateTime(o.tanggal)}
      ...
      <div style="text-align:right;flex-shrink:0">
        <div style="font-size:14px;font-weight:800;font-family:var(--mono);
                    color:var(--accent2)">${fmtRp(o.total)}</div>
        <button onclick="dashboardHapusOmzet('${o.id}')"
          style="margin-top:4px;background:var(--red)11;border:1px solid var(--red)33;
...
export function hapusOmzet(id) {
  if (!confirm('Hapus catatan omzet ini?')) return;
  TxSvc.removeOmzetLog(id);
  const item = document.getElementById(`omzet-item-${id}`);
  if (item) {
...
export function clearOmzet() {
  if (!confirm('Hapus SEMUA riwayat omzet?\nAngka dashboard akan kembali ke 0.')) return;
  TxSvc.clearOmzetLog();
  _render();
}import * as TxSvc  from '../modules/transaksi.service.js';
import * as BrgSvc from '../modules/barang.service.js';
...
function _render() {
  ...
  const omzetLogs    = [...(window.__db?.omzet_log ?? [])].reverse().slice(0, 20);
  const omzetLogHtml = omzetLogs.length
    ? omzetLogs.map(o => _omzetItem(o)).join('')
    : `<div style="font-size:12px;color:var(--muted);padding:16px 0;text-align:center">
         Belum ada catatan omzet
       </div>`;
  ...
  document.getElementById('content').innerHTML = `
    <div class="stat-grid fade-in">
      ...
      <div class="stat-card s-green">
        <div class="stat-label">Cash Hari Ini</div>
        <div class="stat-val" style="font-size:16px;color:var(--green)">${fmtRp(today.cash)}</div>
        <div class="stat-sub">masuk kas</div>
      </div>
      <div class="stat-card s-red">
        <div class="stat-label">Stok Kritis</div>
    ...
    <div class="card fade-in" style="margin-bottom:14px">
      ...
      <div id="omzet-log-list">${omzetLogHtml}</div>
    </div>
...
export function hapusOmzet(id) {
  if (!confirm('Hapus catatan omzet ini?')) return;
  TxSvc.removeOmzetLog(id);
  const el = document.getElementById(`omzet-item-${id}`);
