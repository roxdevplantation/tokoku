...
import { showModal, closeModal }   from '../components/ui.js';
...
function _renderLog() {
  ...
  const countCash   = filtered.filter(t => t.metode === 'cash').length;
  const countKredit = filtered.filter(t => t.metode === 'credit').length;
  ...
  document.getElementById('content').innerHTML = `
    ...
    ${filterChips(FILTER_OPTIONS, _filter, 'logSetFilter')}
    ${filterChips(RANGE_OPTIONS,  _range,  'logSetRange')}
    ...
    <div class="stat-grid fade-in" style="margin-bottom:14px">
      ...
      <div class="stat-card s-green">
        <div class="stat-label">Cash / Kredit</div>
        <div class="stat-val" style="font-size:15px">${countCash} / ${countKredit}</div>
...
function _logItem(tx) {
  const itemsLabel = (tx.items ?? [])
    .map(i => `${escHtml(i.nama)} ×${i.qty}`)
    .join(', ');
